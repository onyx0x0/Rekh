...
class LectureNavigator {
  ...
  init() {
    ...
    const existingNavigation = document.querySelectorAll('.side-nav');
    ...
    const existingQuizBtn = document.querySelector('.quiz-button-container');
    if (existingQuizBtn) {
      existingQuizBtn.remove();
    }

    // Get all lecture segments
    this.segments = Array.from(document.querySelectorAll('.lecture-segment'));
  ...
  createSideNavigation() {
    ...
    const existingNavs = document.querySelectorAll('.side-nav');
   ...
   */
  goToSegment(index) {
    if (index < 0 || index >= this.totalSegments) {
