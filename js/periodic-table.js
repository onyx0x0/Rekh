...
document.addEventListener('DOMContentLoaded', function() {
  // Add periodic table button to header only if it doesn't already exist
  const navLeft = document.querySelector('.nav-left');
  const existingButton = document.getElementById('periodic-table-button');
  
  if (navLeft && !existingButton) {
    ...
    periodicTableButton.addEventListener('click', openPeriodicTableModal);
  } else if (existingButton) {
    // Add event listener to existing button if it's already in the DOM
    existingButton.addEventListener('click', openPeriodicTableModal);
  }
